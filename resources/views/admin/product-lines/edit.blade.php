...
@section('title', 'Edit Product Line - BH Cabinetry Admin Panel')

@section('content')
<div class="p-6 sm:p-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit Product Line</h1>
        <p class="text-gray-600 mt-1">Update the product line information.</p>
    </div>

    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('admin.product-lines.update', $productLine) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="door_style_id" id="door_style_id" 
                            ...
                            @foreach($doorStyles as $doorStyle)
                                <option value="{{ $doorStyle->id }}" {{ old('door_style_id', $productLine->door_style_id) == $doorStyle->id ? 'selected' : '' }}>
                                    {{ $doorStyle->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="door_color_id" id="door_color_id" 
                            ...
                            @foreach($doorColors as $doorColor)
                                <option value="{{ $doorColor->id }}" {{ old('door_color_id', $productLine->door_color_id) == $doorColor->id ? 'selected' : '' }}>
                                    {{ $doorColor->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @foreach($categories as $category)
                            <label class="flex items-center">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}" 
                                       {{ in_array($category->id, old('categories', $productLine->categories->pluck('id')->toArray())) ? 'checked' : '' }}
