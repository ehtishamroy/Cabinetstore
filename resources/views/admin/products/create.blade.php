...
@section('title', 'Create Product - BH Cabinetry Admin Panel')
            ...
            <form action="{{ route('admin.products.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="product_line_id" id="product_line_id" 
                            ...
                            @foreach($productLines as $productLine)
                                <option value="{{ $productLine->id }}" {{ old('product_line_id') == $productLine->id ? 'selected' : '' }}>
                            ...
                            @foreach($subCategories as $subCategory)
                                <label class="flex items-center">
                                    <input type="checkbox" name="sub_categories[]" value="{{ $subCategory->id }}" 
                                           {{ in_array($subCategory->id, old('sub_categories', [])) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <input type="number" name="assembly_cost" id="assembly_cost" value="{{ old('assembly_cost') }}" 
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" 
                               step="0.01" min="0"
                               placeholder="0.00">
                        @error('assembly_cost')
                    ...
                    <div>
                        ...
                        <input type="text" name="hinge_type" id="hinge_type" value="{{ old('hinge_type') }}" 
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" 
