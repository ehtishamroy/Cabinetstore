...
<div class="container mx-auto px-6 py-8">
    ...
    <form action="{{ route('admin.blog.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="excerpt" name="excerpt" rows="3" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent">{{ old('excerpt', $post->excerpt) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="15" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent" required>{{ old('content', $post->content) }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="category" name="category" 
                        ...
                        <option value="Design Trends" {{ old('category', $post->category) == 'Design Trends' ? 'selected' : '' }}>Design Trends</option>
                        <option value="DIY Guides" {{ old('category', $post->category) == 'DIY Guides' ? 'selected' : '' }}>DIY Guides</option>
                        <option value="Small Spaces" {{ old('category', $post->category) == 'Small Spaces' ? 'selected' : '' }}>Small Spaces</option>
                        <option value="Maintenance" {{ old('category', $post->category) == 'Maintenance' ? 'selected' : '' }}>Maintenance</option>
                        <option value="Color Theory" {{ old('category', $post->category) == 'Color Theory' ? 'selected' : '' }}>Color Theory</option>
                        <option value="Details" {{ old('category', $post->category) == 'Details' ? 'selected' : '' }}>Details</option>
                ...
                <div>
                    ...
                    <input type="number" id="read_time" name="read_time" value="{{ old('read_time', $post->read_time) }}" min="1" max="60" 
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent" required>
                </div>
                ...
                <div class="border-t pt-6">
                    ...
                    <div>
                        ...
                        <textarea id="meta_description" name="meta_description" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent">{{ old('meta_description', $post->meta_description) }}</textarea>
